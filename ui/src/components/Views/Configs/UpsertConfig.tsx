...
import { AUTH_TOKEN_VAR } from '../../../constants';
...
const COMMON_CONFIGURATIONS = [
  ...
  [AUTH_TOKEN_VAR, '', 'Auth token to activate LocalStack Pro.'],
];
...
export const UpsertConfig = ({
  ...
  const [newConfig, setNewConfig] = useState<RunConfig>(
    config ||
      ({
        name: '',
        id: uuid(),
        vars: COMMON_CONFIGURATIONS.map(([variable, value, description]) => ({
          variable,
          value,
          id: uuid(),
          description,
  ...
  const handleRemoveButtonPress = (id: string) => {
    setNewConfig({
      name: newConfig.name,
      id: newConfig.id || uuid(),
      vars: newConfig.vars?.filter((item) => item?.id !== id) || [],
    });
  };
  ...
  const updateConfigKey = (id: string, key: string) => {
    setNewConfig({
      ...newConfig,
      vars: newConfig.vars.map((envVar) => {
        if (envVar.id === id) {
  ...
  const hasSetAuthToken = newConfig.vars.some((v) => v.variable === AUTH_TOKEN_VAR && v.value !== '');
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      <DialogContent>
        ...
        <Box className={classes.emptyBox} />
        <List subheader={<Typography>Environment Variables</Typography>}>
          {newConfig?.vars.map((item) => {
            const isAuthTokenEnv = item.variable === AUTH_TOKEN_VAR;
            return (
                ...
                <Box width={DEFAULT_COLUMN_WIDTH}>
                  <Box display="flex" key={item.id}>
                    ...
                    <TextField
                      ...
                      value={item.value}
                      required={isAuthTokenEnv}
                    />
                    <IconButton onClick={() => handleRemoveButtonPress(item.id)} disabled={isAuthTokenEnv}>
                      <Remove />
                    </IconButton>
                  </Box>
                  {item.description && (
      ...
      <DialogActions>
        ...
        <Button
          ...
          disabled={!configName || !hasSetAuthToken}
        >
          Save & Exit
        </Button>
      </DialogActions>
